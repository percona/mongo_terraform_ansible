...
3. Run the playbook, specifying the bucket for storing the backups, and a key pair with perms for accessing the bucket (you can use the one generated by Terraform)
```
ansible-playbook main.yml -i inventory --extra-vars "bucket=$env_name-mongo-backups access_key_id=***** secret_access_key=******"
```
