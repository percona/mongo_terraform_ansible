...
resource "aws_instance" "pmm" {
  ...
  vpc_security_group_ids = [ aws_security_group.allow_ssh.id, aws_security_group.allow_all_outbound.id, aws_security_group.allow_ping.id, aws_security_group.mongodb_pmm_sg.id ]
  user_data = <<-EOT
    #!/bin/bash
    # Set the hostname
