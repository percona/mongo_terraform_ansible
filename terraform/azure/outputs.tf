  ...
  content = templatefile("cluster_inventory.tmpl",
    {
      ...
      ldap_ip              = var.clusters[each.key].ldap_server != "" ? azurerm_linux_virtual_machine.ldap[var.clusters[each.key].ldap_server].public_ip_address : ""
      ldap_endpoint        = var.clusters[each.key].ldap_server != "" ? azurerm_linux_virtual_machine.ldap[var.clusters[each.key].ldap_server].private_ip_address : ""
  ...
  content = templatefile("replset_inventory.tmpl",
    {
      ...
      ldap_ip              = var.replsets[each.key].ldap_server != "" ? azurerm_linux_virtual_machine.ldap[var.replsets[each.key].ldap_server].public_ip_address : ""
      ldap_endpoint        = var.replsets[each.key].ldap_server != "" ? azurerm_linux_virtual_machine.ldap[var.replsets[each.key].ldap_server].private_ip_address : ""
