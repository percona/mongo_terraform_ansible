...
resource "docker_container" "pmm" {
  ...
  env = [ "GF_RENDERING_SERVER_URL=http://${docker_container.renderer.name}:${var.renderer_port}/render", "GF_RENDERING_CALLBACK_URL=https://${var.pmm_tag}:${var.pmm_port}/graph/" ]
  mounts {
    type = "volume"
    target = "/srv"
  ...
  healthcheck {
    test        = ["CMD", "curl", "-k", "-f", "https://${var.pmm_tag}:${var.pmm_port}/graph/login" ]
    interval    = "10s"
    timeout     = "10s"
    retries     = 5
    start_period = "30s"
